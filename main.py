...
def directory_brute_force(url, directory, output_file, proxies=None, white_list=None, black_list=None, timeout=5):
    ...
    try:
        ...
        logging.error(f"Timeout - {full_url}")
        return
    except requests.ConnectionError:
        logging.error(f"Connection Error - {full_url}")
        return
    except requests.RequestException as e:
        logging.error(f"Error - {e} - {full_url}")
        return
    ...
    if white_list and status in white_list:
        ...
        logging.info(f"Whitelist Match - {full_url}")
        ...
        logging.info(f"Successful Hit - {full_url}")
...
def start_threads(url, directories, output_file, proxies=None, thread_count=1, white_list=None, black_list=None, timeout=5):
    ...
    def threaded_directory_brute_force(directory):
        directory_brute_force(url, directory, output_file,
                              proxies, white_list, black_list, timeout)
        with progress_lock:
            nonlocal completed
            completed += 1
            progress = int((completed / total_directories)
                           * progress_bar_width)
    ...
    for directory in directories:
        thread = threading.Thread(
            target=threaded_directory_brute_force, args=(directory,))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Multi-Threaded Directory Brute Force")
    ...
    parser.add_argument("-p", "--proxies", type=str, default=None,
                        help="Proxy settings (e.g., http://proxy-server:port)")
    parser.add_argument("-t", "--threads", type=int, default=1,
                        help="Number of threads (default: 1)")
    parser.add_argument("-w", "--whitelist", type=int, nargs="*",
                        help="Whitelist of HTTP status codes (e.g., 200 204 301)")
    parser.add_argument("-b", "--blacklist", type=int, nargs="*",
                        help="Blacklist of HTTP status codes to ignore")
    parser.add_argument("-o", "--output", type=str,
                        default="dir.txt", help="Output file name")
    ...
    proxy_settings = {'http': args.proxies,
                      'https': args.proxies} if args.proxies else None
    ...
    logging.info("Starting Multi-Threaded Directory Brute Force")
    logging.info(f"Target URL: {target_url}")
    logging.info(f"Word List: {wordlist_path}")
    logging.info(f"Proxy Settings: {proxy_settings}")
    logging.info(f"Threads: {thread_count}")
    logging.info(f"Whitelist: {white_list}")
    logging.info(f"Blacklist: {black_list}")
    ...
    try:
        ...
        logging.error("Error: Unable to read the word list file.")
        sys.exit(1)
    ...
    start_threads(target_url, directories, output_file,
                  proxies=proxy_settings, thread_count=thread_count)

    logging.info(
        "Directory Brute Force completed. Results written to the output file.")
